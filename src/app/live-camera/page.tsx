...
export default function LiveCameraPage() {
  ...
  const [role, setRole] = useState<"host" | "viewer">("host");
  ...
  useEffect(() => {
    ...
    const requestedRole = params.get("role") === "viewer" ? "viewer" : "host";
  ...
  const viewerUrl = typeof window !== "undefined" && room ? `${window.location.origin}/live-camera?room=${encodeURIComponent(room)}&role=viewer` : "";
  const qrUrl = viewerUrl ? `https://api.qrserver.com/v1/create-qr-code/?size=220x220&data=${encodeURIComponent(viewerUrl)}` : "";
